...
mkdir -p /package_build/opencv-3.4.1/etc/ld.so.conf.d
echo "/package_build/opencv-3.4.1/usr/local/lib" > /package_build/opencv-3.4.1/etc/ld.so.conf.d/opencv.conf


mkdir -p /package_build/opencv-3.4.1/DEBIAN \
&& cd /package_build \
&& echo -e "Source: opencv-3.4.1\n\
Package: opencv\n\
Version: 3.4.1\n\
Priority: optional\n\
...
Architecture: arm64\n\
Depends: \n\
Description: OpenCV version 3.4.1\n"\
> /package_build/opencv-3.4.1/DEBIAN/control \
&& fakeroot dpkg-deb --build opencv-3.4.1

mkdir -p $SCRIPT_DIR/../binary
mv -f /package_build/opencv-3.4.1.deb $SCRIPT_DIR/../binary
