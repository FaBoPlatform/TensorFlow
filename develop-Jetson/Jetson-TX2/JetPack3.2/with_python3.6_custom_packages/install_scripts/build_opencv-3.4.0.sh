...
mkdir -p /compile \
&& cd /compile \
&& wget --no-check-certificate https://github.com/opencv/opencv/archive/3.4.0.zip \
&& unzip 3.4.0.zip \
&& mkdir -p opencv-3.4.0/build \
&& cd opencv-3.4.0/build

time cmake -D CMAKE_C_FLAGS="-std=c11 -march=native" -D CMAKE_CXX_FLAGS="-march=native" -D ENABLE_CXX11=ON -D CMAKE_BUILD_TYPE=RELEASE -D CMAKE_INSTALL_PREFIX=/package_build/opencv-3.4.0/usr/local -D WITH_TBB=ON -D WITH_V4L=ON -D WITH_QT=ON -D WITH_OPENGL=ON -D WITH_CUBLAS=ON -D WITH_CUDA=ON -D CUDA_ARCH_BIN="6.2" -D CUDA_ARCH_PTX="6.2" -D ENABLE_FAST_MATH=ON -D CUDA_FAST_MATH=ON -D CUDA_NVCC_FLAGS="-D_FORCE_INLINES" -D CUDA_CUDA_LIBRARY=/usr/lib/aarch64-linux-gnu/libcuda.so -D WITH_GDAL=ON -D WITH_XINE=ON -D BUILD_EXAMPLES=ON -D PYTHON3_EXECUTABLE=/usr/bin/python3 -D PYTHON_INCLUDE_DIR=/usr/include/python3.6 -D PYTHON_INCLUDE_DIR2=/usr/include/aarch64-linux-gnu/python3.6m -D PYTHON_LIBRARY=/usr/lib/aarch64-linux-gnu/libpython3.6m.so -D PYTHON3_NUMPY_INCLUDE_DIRS=/usr/local/lib/python3.6/dist-packages/numpy/core/include/ ..

time make install -j $(($(nproc) + 1))

mkdir -p /package_build/opencv-3.4.0/etc/ld.so.conf.d
echo "/package_build/opencv-3.4.0/usr/local/lib" > /package_build/opencv-3.4.0/etc/ld.so.conf.d/opencv.conf


mkdir -p /package_build/opencv-3.4.0/DEBIAN \
&& cd /package_build \
&& echo -e "Source: opencv-3.4.0\n\
Package: opencv\n\
Version: 3.4.0\n\
Priority: optional\n\
...
Architecture: arm64\n\
Depends: \n\
Description: OpenCV version 3.4.0\n"\
> /package_build/opencv-3.4.0/DEBIAN/control \
&& fakeroot dpkg-deb --build opencv-3.4.0

mkdir -p $SCRIPT_DIR/../binary
mv -f /package_build/opencv-3.4.0.deb $SCRIPT_DIR/../binary
